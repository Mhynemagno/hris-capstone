...
HR Personnel register a consenting employee's face at `/hr/attendance/face-enrollment` and run a supervised kiosk at `/hr/attendance/kiosk`. At the kiosk an employee looks at the camera and blinks once; the database matches the face and records time-in (first scan of the day) or time-out (a later scan), using the same status rules as imports. Details, thresholds, and security model: [`docs/superpowers/specs/2026-09-25-face-recognition-attendance-design.md`](docs/superpowers/specs/2026-09-25-face-recognition-attendance-design.md).
