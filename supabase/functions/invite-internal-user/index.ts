...
Deno.serve(async (request) => {
  ...
  if (!parsed.success) {
    return json(400, { error: "Invalid invitation details." });
  }
  ...
  const { data: roleRow } = await callerClient.from("user_roles").select("role")
  ...
  const redirectTo = getInvitationRedirectUrl(Deno.env.get("APP_URL"));
  if (!redirectTo) {
    return json(500, { error: "Invitation redirects are not configured." });
  }
