...
Deno.test("processes a queued application without returning document content", async () => {
  ...
  const client = {
    rpc: async (name: string, args: Record<string, unknown>) => {
      assertEquals(name, "complete_application_analysis");
    ...
    schema: () => ({
      rpc: async (name: string, args: Record<string, unknown>) => {
        if (name === "read") {
          assertEquals(args, { queue_name: "application_analysis", visibility_timeout_seconds: 600, n: 5 });
  ...
  assertEquals(updates.map((entry) => entry.values.status), ["processing"]);
  assertEquals(completionCalls, [{
    target_score_id: scoreId,
    target_score: 86,
    target_explanation: "Meets the required experience criterion.",
    target_provider: "gemini",
    target_model: "gemini-2.5-flash-lite",
    target_model_version: "v1beta",
    target_completed_at: "2026-08-27T00:00:00.000Z",
  }]);
  assertEquals(JSON.stringify(completionCalls).includes("AQID"), false);
});
...
Deno.test("requeues a transient provider failure once before deleting its message", async () => {
  ...
  assertEquals(queueCalls, [
    { name: "read", args: { queue_name: "application_analysis", visibility_timeout_seconds: 600, n: 5 } },
...
Deno.test("keeps a terminal message when the failed state cannot be persisted", async () => {
  const queueCalls: string[] = [];
  let updateCount = 0;
  const client = {
    schema: () => ({
      rpc: async (name: string) => {
        queueCalls.push(name);
        if (name === "read") return { data: [{ msg_id: 10n, message: { scoreId } }], error: null };
        return { data: true, error: null };
      },
    }),
    from: (table: string) => ({
      update() {
        ...
        const query = {
          ...
          then: (resolve: (value: { data: null; error: { message: string } | null }) => unknown) =>
            Promise.resolve(updateCount === 1 ? { data: null, error: null } : { data: null, error: { message: "database unavailable" } }).then(resolve),
        };
        return query;
      },
      select: () => ({
