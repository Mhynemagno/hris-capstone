...
export function ForgotPasswordForm() {
  ...
  return <form className="space-y-4" onSubmit={onSubmit} noValidate><FormField htmlFor="recovery-email" label="Email"><input className="w-full rounded-md border bg-white px-3 py-2 text-slate-950" id="recovery-email" name="email" type="email" /></FormField>{error ? <ErrorState message={error} /> : null}<button className="w-full rounded-md bg-primary px-4 py-2 font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-60" disabled={pending} type="submit">{pending ? "Sending…" : "Send reset link"}</button></form>;
}
