...
export function EmployeeDirectory() {
  ...
  return <>
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><label className="sr-only" htmlFor="employee-search">Search employees</label><input className="h-11 rounded-lg border border-input bg-background px-3 text-sm sm:w-80" id="employee-search" onChange={(event) => setSearch(event.target.value)} placeholder="Search name or employee number" value={search} /><Link className="inline-flex h-11 w-full items-center justify-center rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground sm:w-auto" href="/hr/employees/new">Add employee</Link></div>
