...
export function MyApplicationList() {
  ...
  return <div className="space-y-3">{rows.map((application) => <Link className="block rounded-xl border p-4 hover:bg-muted" href={`/applicant/applications/${application.id}`} key={application.id}><p className="font-medium">Application {application.id.slice(0, 8)}</p><p className="mt-1 text-sm text-muted-foreground">Status: {application.status}</p></Link>)}</div>;
}
