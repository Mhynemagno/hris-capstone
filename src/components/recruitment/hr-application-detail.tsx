...
function AnalysisRecommendation({ applicationId, score, onError }: { applicationId: string; score: ApplicationAiScore | undefined; onError: (message: string) => void }) {
  ...
  return <section aria-live="polite" className="rounded-xl border border-primary/30 bg-muted p-5">
    ...
    {score?.status === "failed" ? <div className="mt-4 space-y-3"><p className="text-sm">Analysis could not be completed. You can try it once more.</p><button className="rounded-lg border px-4 py-2 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-60" disabled={retry.isPending} onClick={() => void retryAnalysis()} type="button">{retry.isPending ? "Retrying analysis…" : "Retry analysis"}</button></div> : null}
