...
export function PublicJobDetail({ jobId }: { jobId: number }) {
  ...
  return <section className="mx-auto max-w-3xl space-y-6"><div><Link className="text-sm text-primary underline-offset-4 hover:underline" href="/jobs">All job openings</Link><p className="mt-4 text-sm text-muted-foreground">{job.data.location || "Location to be confirmed"}</p><h1 className="text-3xl font-semibold tracking-tight">{job.data.title}</h1><p className="mt-4 whitespace-pre-wrap text-muted-foreground">{job.data.description}</p></div><div className="rounded-xl border p-5"><h2 className="font-semibold">Qualification criteria</h2><ul className="mt-3 list-disc space-y-2 pl-5 text-sm">{criteria.map((criterion) => <li key={criterion.id}><span className="font-medium">{criterion.is_required ? "Required" : "Preferred"}:</span> {criterion.requirement}</li>)}</ul></div><div className="flex flex-wrap gap-3"><Link className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground" href={`/applicant/applications?jobId=${job.data.id}`}>Apply for this opening</Link><Link className="rounded-lg border px-4 py-2 text-sm font-medium" href="/applicant/register">Create applicant account</Link></div></section>;
}
